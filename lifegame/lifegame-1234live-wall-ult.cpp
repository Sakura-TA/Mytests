...
public:
	...
	int calcSurround(int y,int x)
	{
		...
		for(int i=0;i<8;i++)
		{
			if(x+dir[i][0]<0||x+dir[i][0]>=width||y+dir[i][1]<0||y+dir[i][1]>=height) continue;
			if(mp[y+dir[i][1]][x+dir[i][0]]>1) c++;
		}
		return c;
	}
	lg()
	{
		...
		for(int i=0;i<height;i++)
		{
			...
			{
				if(i>=w&&i<height-w&&j>=w&&j<width-w) mp[i][j]=0;
				else mp[i][j]=rand()&1<<1;
				if(mp[i][j]) lives++;
			}
		}
	}
	...
	lg(lg *t)
	{
		...
		for(int i=0;i<height;i++)
		{
			for(int j=0;j<width;j++)
			{
				...
				if(t->mp[i][j]>1)
				{
					if(c==1||c==2) mp[i][j]=2,lives++;
					else mp[i][j]=1;
				}					// 0 死 1新死 2生 3新生
				else if(c==1||c==2) mp[i][j]=3,lives++;
				else mp[i][j]=0;
			}
		}
		gene=t->gene+1;
